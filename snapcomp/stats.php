...
{
	$db = Db::getInstance();
	if ($stmt = mysqli_prepare($db, "SELECT ID, COUNT(ID) as CNT FROM (SELECT ID, ( 3959 * acos( cos( radians(?) ) * cos( radians( LATITUDE ) ) * cos( radians( LONGITUDE ) - radians(?) ) + sin( radians(?) ) * sin( radians( LATITUDE ) ) ) ) AS distance FROM PICTURE HAVING distance < 1005) as res GROUP BY ID ORDER BY CNT;"))
	{
